...
def main():
    ap=argparse.ArgumentParser(); ap.add_argument('--model',default='artifacts/rkimage1/model.pt'); ap.add_argument('--out',default='artifacts/Rkimage-1-f16.gguf'); ap.add_argument('--quant',choices=['f16','q8_0'],default='f16'); args=ap.parse_args()
    ck=torch.load(args.model,map_location='cpu'); w=GGUFWriter(args.out,'t2i-diffusion'); c=ck['config']; w.add_string('general.architecture','t2i-diffusion'); w.add_string('general.name','Rkimage 1'); w.add_string('general.description','Rkimage 1 custom latent diffusion transformer trained with ma-xu/fine-t2i'); w.add_string('t2i.dataset','ma-xu/fine-t2i'); w.add_string('t2i.config',json.dumps(c)); w.add_uint64('t2i.trainable_parameters',sum(v.numel() for v in ck['model'].values()))
